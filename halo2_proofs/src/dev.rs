                                ...
                                Value::Real(_) => {
                                   let load_fn = |n: i32, row: i32| {
                                       for (column, at) in self.cs.fixed_queries.iter() {
                                              ...
                                              println!("hehe, not constraint, fixed: row={}, col={:?}, rot={:?}, val={:?}", resolved_row, column, at, val);
                                       }
                                       for (column, at) in self.cs.advice_queries.iter() {
                                              let resolved_row = (row + at.0) % n;
                                              let val = self.advice[column.index()][resolved_row as usize];
                                              println!("hehe, not constraint, advice: row={}, col={:?}, rot={:?}, val={:?}", resolved_row, column, at, val);
                                       }
                                    };
                                    load_fn(n, row);
                                ...
                                Value::Poison => {
                                   let load_fn = |n: i32, row: i32| {
                                       for (column, at) in self.cs.fixed_queries.iter() {
                                              let resolved_row = (row + at.0) % n;
                                              let val = self.fixed[column.index()][resolved_row as usize];
                                              println!("hehe, poisoned, fixed: row={}, col={:?}, rot={:?}, val={:?}", resolved_row, column, at, val);
                                       }
                                       for (column, at) in self.cs.advice_queries.iter() {
                                              let resolved_row = (row + at.0) % n;
                                              let val = self.advice[column.index()][resolved_row as usize];
                                              println!("hehe, poisoned, advice: row={}, col={:?}, rot={:?}, val={:?}", resolved_row, column, at, val);
                                       }
                                    };
                                    load_fn(n, row);
